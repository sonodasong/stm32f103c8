...
const uint8 dmpConfig[MPU6050_DMP_CONFIG_SIZE] = {
	...
	0x02, 0x16, 0x02, 0x00, FIFO_RATE // D_0_22 inv_set_fifo_rate
...
uint8 MPU6050dmpInitialize(void)
{
	...
	if (MPU6050writeProgMemoryBlock(dmpMemory, MPU6050_DMP_CODE_SIZE, 0, 0, TRUE)) {
		if (MPU6050writeProgDMPConfigurationSet(dmpConfig, MPU6050_DMP_CONFIG_SIZE)) {
			...
			/* according to experiment, setting offset have better performance than no setting offset */
			MPU6050setXGyroOffset(xgOffsetTC);
			MPU6050setYGyroOffset(ygOffsetTC);
			MPU6050setZGyroOffset(zgOffsetTC);
			...
			writeByte(MPU6050_ADDRESS_COMPASS, 0x0A, 0x01);
			/* remember to change address */
			writeByte(MPU6050_ADDRESS, MPU6050_RA_I2C_SLV0_ADDR, 0x8C);
			writeByte(MPU6050_ADDRESS, MPU6050_RA_I2C_SLV0_REG, 0x01);
			writeByte(MPU6050_ADDRESS, MPU6050_RA_I2C_SLV0_CTRL, 0xDA);
			/* remember to change address */
			writeByte(MPU6050_ADDRESS, MPU6050_RA_I2C_SLV2_ADDR, 0x0C);
			...
			/* AK8963 sample rate */
			writeByte(MPU6050_ADDRESS, MPU6050_RA_I2C_SLV4_CTRL, FIFO_RATE);
...
static void mpu6050DMPHandler(void);

void mpu6050DMPInit(void)
{
	mpu6050IRQInit(mpu6050DMPHandler);
